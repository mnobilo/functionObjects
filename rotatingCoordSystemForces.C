...
void Foam::functionObjects::rotatingCoordSystemForces::rotateCS()
{
    ...
    Info<< "Rotational speed" << " " << omega << endl; //write out omega for check
    
    //read the runner's axis of rotation
    ...
    Info<< "Current angle of rotation" << " " << angle << endl; //write out the angle for check
    ...
    if (axisLocal_ == "e1")
    {
        const vector& axisRot = coordSysPtr_->e1(); //it can be the current axis as well, it is just the direction of the rotation
        Info<< "Local axis of rotation" << " " << axisRot << endl;
        rotTensor0_ = quaternion(axisRot,-angle).R(); //rotational tensor saved in memner data
    }
    if (axisLocal_ == "e2")
    {
    ...
    tensor rotTensor(rotTensor0_); //rotational matrix

    Info<< "Rotation tensor" << rotTensor << endl;

    //Rotation of coordinate system axes (overwrite the original coordinate system)
    vector e1 =  rotTensor & e1_;
    //vector e3 = e3_ & rotTensor;
    vector e3 = rotTensor & e3_;
    //coordSysPtr_.reset(new coordSystem::cartesian(originNew, coordSysPtr_->e3() & rotTensor, coordSysPtr_->e1() & rotTensor);
