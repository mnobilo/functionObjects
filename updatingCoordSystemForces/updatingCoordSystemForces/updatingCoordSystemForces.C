...
#include "updatingCoordSystemForces.H"
#include "fvcGrad.H"
...
{
    defineTypeNameAndDebug(updatingCoordSystemForces, 0);
    addToRunTimeSelectionTable(functionObject, updatingCoordSystemForces, dictionary);
}
}
...
void Foam::functionObjects::updatingCoordSystemForces::setCoordinateSystem
(
...
Foam::volVectorField& Foam::functionObjects::updatingCoordSystemForces::force()
{
...
Foam::volVectorField& Foam::functionObjects::updatingCoordSystemForces::moment()
{
...
void Foam::functionObjects::updatingCoordSystemForces::initialise()
{
    if (initialised_)
    {
        return;
...
void Foam::functionObjects::updatingCoordSystemForces::reset()
{
...
Foam::functionObjects::updatingCoordSystemForces::devRhoReff() const
{
...
Foam::tmp<Foam::volScalarField> Foam::functionObjects::updatingCoordSystemForces::mu() const
{
...
Foam::tmp<Foam::volScalarField> Foam::functionObjects::updatingCoordSystemForces::rho() const
{
    if (rhoName_ == "rhoInf")
    {
...
Foam::scalar Foam::functionObjects::updatingCoordSystemForces::rho(const volScalarField& p) const
{
...
void Foam::functionObjects::updatingCoordSystemForces::addToPatchFields
(
    const label patchi,
...
void Foam::functionObjects::updatingCoordSystemForces::addToInternalField
(
...
void Foam::functionObjects::updatingCoordSystemForces::createIntegratedDataFiles()
{
...
void Foam::functionObjects::updatingCoordSystemForces::writeIntegratedDataFileHeader
(
    const word& header,
    OFstream& os
) const
...
void Foam::functionObjects::updatingCoordSystemForces::writeIntegratedDataFiles()
{
    const auto& coordSys = coordSysPtr_();
...
void Foam::functionObjects::updatingCoordSystemForces::writeIntegratedDataFile
(
    const vector& pres,
    const vector& vis,
...
void Foam::functionObjects::updatingCoordSystemForces::logIntegratedData
(
...
Foam::functionObjects::updatingCoordSystemForces::updatingCoordSystemForces
(
    const word& name,
    const Time& runTime,
    const dictionary& dict,
...
(
    ...
    const dictionary& dict,
    //const dictionary& SBMFCoeffs, //added
    const Time& runTime,
    bool readFields
)
:
    ...
    omega_(SBMFCoeffs_.get<scalar>("omega")),*/
...
bool Foam::functionObjects::updatingCoordSystemForces::read(const dictionary& dict)
{
...
void Foam::functionObjects::updatingCoordSystemForces::calcForcesMoments()
{
    initialise();
    ...
    scalar omega (readScalar(dynamicMesh.lookup("omega")));
   
    //auto omega = Function1<scalar>::New("omega", dynamicMesh); 
    Info<< "Rotational speed" << " " << omega << endl;


    //Info<< "SBMFCoeffs" << " " << SBMFCoeffs_ << endl;
    //Read axis of rotation for rotating origin
    vector axis = vector(dynamicMesh.lookup("axis") );

    Info<< "Axis of rotation" << " " << axis << endl;

    //Rotation angle for origin
    scalar angle = omega*t;
    //scalar angle = omega->integrate(0, t);
    
    Info<< "Current angle of rotation" << " " << angle << endl;

    const point& originOrig = coordSysPtr_->origin();
    
    Info<< "Original origin" << " " << originOrig << endl;
    

    if (axis == vector(0,0,1))    
    {
        //scalar r = sqrt(pow(originOrig[0],2)+pow(originOrig[1],2));
    ...
    {
	//scalar r = sqrt(pow(originOrig[0],2)+pow(originOrig[2],2));
    ...
    {
	//scalar r = sqrt(pow(originOrig[1],2)+pow(originOrig[2],2));
    ...
    point origin(xNew_, yNew_, zNew_);
    Info<< "New origin" << " " << origin << endl;
    ...
    else
    {
        ...
        for (const label patchi : patchSet_)
        {
	        ...
	        const vectorField Md0(d - origin);

 	        const vectorField Md(Md0 & rotTensor);



            const vectorField fP
            (
                rhoRef*(Sfb[patchi] & rotTensor)*(p.boundaryField()[patchi] - pRef)
            );
            
	        const auto& devRhoReffbRot = devRhoReffb[patchi] & rotTensor;
	        const vectorField fV(Sfb[patchi] & devRhoReffbRot);
...
Foam::vector Foam::functionObjects::updatingCoordSystemForces::forceEff() const
{
    return sumPatchForcesP_ + sumPatchForcesV_ + sumInternalForces_;
}


Foam::vector Foam::functionObjects::updatingCoordSystemForces::momentEff() const
{
    return sumPatchMomentsP_ + sumPatchMomentsV_ + sumInternalMoments_;
}


bool Foam::functionObjects::updatingCoordSystemForces::execute()
{
    calcForcesMoments();
    ...
    logIntegratedData("updatingCoordSystemForces", localFp, localFv, localFi);
...
bool Foam::functionObjects::updatingCoordSystemForces::write()
{
    if (writeToFile())
    {
